...
class ExtensionFilesTest(unittest.TestCase):
    def test_background_relays_local_requests(self):
        ...
        self.assertIn('chrome.tabs.query({ url: ["https://photos.google.com/*"] }', background)
        self.assertIn("chrome.tabs.reload(tab.id", background)
        ...
        self.assertIn('<option value="trash" selected>Papelera (predeterminado)</option>', content)
        self.assertIn('"ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"', content)
        self.assertIn('"View previous photo"', content)
        self.assertIn("findPreviousNavigationButton", content)
        self.assertIn("requestPreviousPhoto", content)
        self.assertIn("goToPreviousPhoto", content)
        self.assertIn("previousNavigationRequested", content)
        self.assertIn('id="swipeclean-previous"', content)
        self.assertIn('event.key === "ArrowUp"', content)
        ...
        self.assertIn("queuedTrashCount", content)
        self.assertIn("requestDiscardCurrentPhoto", content)
        self.assertIn("flushQueuedDecision", content)
        self.assertIn("updateDiscardControl", content)
        self.assertIn("en espera", content)
        self.assertIn("watchForTrashConfirmation", content)
